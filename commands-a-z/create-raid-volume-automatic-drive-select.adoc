...
---
= RAID-Volume erstellen (automatische Laufwerksauswahl) – SANtricity CLI
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Die maximale Länge für Standard- und Thin Volume-Namen beträgt 60 Zeichen. Der Volume-Name kann beliebige Kombinationen aus alphanumerischen Zeichen, Unterstrich (_), Bindestrich (-) und Raute (#) enthalten.
...
`resourceProvisioningCapable`
 a| 
Wenn dieser Parameter auf  `TRUE` gesetzt ist, werden nur DULBE-fähige Laufwerke zum Erstellen von Datenträgerpools oder Volumengruppen verwendet. Wenn der Parameter auf  `FALSE` gesetzt ist oder nicht angegeben wird, erfolgt keine Prüfung auf DULBE-Fähigkeit. Wenn alle für die Volumengruppe oder den Datenträgerpool ausgewählten Laufwerke DULBE-fähig sind, bestimmt das Flag auf Speicherarray-Ebene, ob die Volumengruppe oder der Datenträgerpool als Resource Provisioned erstellt wird.
...
Sie können für die Namen beliebige Kombinationen aus alphanumerischen Zeichen, Bindestrichen und Unterstrichen verwenden. Namen für Standard und Thin Volumes dürfen maximal 60 Zeichen lang sein, alle anderen Komponentennamen maximal 30 Zeichen.
